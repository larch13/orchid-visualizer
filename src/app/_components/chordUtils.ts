...
export type ChordType = 'Major' | 'Minor' | 'Diminished' | 'Augmented' | 'Major 7th' | 'Minor 7th' | 'Dominant 7th' | 'Sus4' | 'Sus2' | '6th' | '9th';
export type Inversion = number[];

// Constants
export const BASE_NOTES = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B'] as const;
...
export const generateVoicings = (baseOffset: number, intervals: number[]): number[] => {
...
export const getVoicingsForNote = (note: NoteName, chordQuality: ChordType): number[] => {
...
export const getFirstVoicingForNote = (note: NoteName, chordQuality: ChordType): number | null => {
...
export const generateFirstVoicingMap = (): Record<NoteName, Record<ChordType, number | null>> => {
  const wholeNotes: NoteName[] = ['C', 'D', 'E', 'F', 'G', 'A', 'B'];
  const result: Record<NoteName, Record<ChordType, number | null>> = {} as Record<NoteName, Record<ChordType, number | null>>;

  wholeNotes.forEach(note => {
    result[note] = {} as Record<ChordType, number | null>;
    Object.keys(CHORD_PATTERNS).forEach(chordType => {
      result[note][chordType as ChordType] = getFirstVoicingForNote(note, chordType as ChordType);
    });
  });
...
export const getColorBrightness = (midiNote: number, baseColor: string): string => {
  const octave = Math.floor(midiNote / 12) - 2;
